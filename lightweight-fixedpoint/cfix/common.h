/*
 * Lightweight fixed-point library for C.
 *
 * Copyright (c) 2008 the handy-eda-utils develeper(s).
 * Distributed under the MIT License.
 * (See accompanying file COPYING or copy at
 *  http://www.opensource.org/licenses/mit-license.php.)
 *
 * Last modified on November 2007.
 */

#ifndef COMMON_H_INCLUDED
#define COMMON_H_INCLUDED

typedef unsigned char bool;
#define	TRUE	1
#define	FALSE	0

typedef signed char int2;
typedef signed char int3;
typedef signed char int4;
typedef signed char int5;
typedef signed char int6;
typedef signed char int7;
typedef signed char int8;
typedef signed short int9;
typedef signed short int10;
typedef signed short int11;
typedef signed short int12;
typedef signed short int13;
typedef signed short int14;
typedef signed short int15;
typedef signed short int16;
typedef signed int int17;
typedef signed int int18;
typedef signed int int19;
typedef signed int int20;
typedef signed int int21;
typedef signed int int22;
typedef signed int int23;
typedef signed int int24;
typedef signed int int25;
typedef signed int int26;
typedef signed int int27;
typedef signed int int28;
typedef signed int int29;
typedef signed int int30;
typedef signed int int31;
typedef signed int int32;

typedef unsigned char uint2;
typedef unsigned char uint3;
typedef unsigned char uint4;
typedef unsigned char uint5;
typedef unsigned char uint6;
typedef unsigned char uint7;
typedef unsigned char uint8;
typedef unsigned short uint9;
typedef unsigned short uint10;
typedef unsigned short uint11;
typedef unsigned short uint12;
typedef unsigned short uint13;
typedef unsigned short uint14;
typedef unsigned short uint15;
typedef unsigned short uint16;
typedef unsigned int uint17;
typedef unsigned int uint18;
typedef unsigned int uint19;
typedef unsigned int uint20;
typedef unsigned int uint21;
typedef unsigned int uint22;
typedef unsigned int uint23;
typedef unsigned int uint24;
typedef unsigned int uint25;
typedef unsigned int uint26;
typedef unsigned int uint27;
typedef unsigned int uint28;
typedef unsigned int uint29;
typedef unsigned int uint30;
typedef unsigned int uint31;
typedef unsigned int uint32;

typedef struct { int re, im; } complex;

#endif
